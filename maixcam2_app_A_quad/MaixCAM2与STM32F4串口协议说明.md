...
> 协议版本：2（10字节会话心跳，不兼容旧版6字节心跳）
> 对应视觉程序：`maixcam2_app_A_quad` v2.0.1
...
视觉应用启动UART4前会显式调用`maix.comm.rm_default_comm_listener()`，停止Maix系统默认通信监听，再映射A21/A22并打开`/dev/ttyS4`。UART打开失败或应用关闭UART时会调用`maix.comm.add_default_comm_listener()`恢复默认监听。应用运行期间UART4由本协议独占，不能再让其他程序同时打开同一设备。
...
/* 从未对齐字节流读取小端uint32，用于心跳会话ID和运行毫秒数。 */
...
| 载荷偏移 | 长度 | 类型 | 字段 |
|---:|---:|---|---|
| 0 | 4 | `uint32` | `session_id`，本次视觉应用启动会话，非0 |
| 4 | 4 | `uint32` | `uptime_ms`，本次应用运行毫秒数，自然回绕 |
| 8 | 1 | `uint8` | `app_state`，见下表 |
| 9 | 1 | `uint8` | `last_error`，当前固定为0 |

| `app_state` | 状态 | F4建议动作 |
|---:|---|---|
| 0 | 完全待机 | 不执行新的拼图动作 |
| 1 | CAL调参 | 禁止机械拼图动作，可接收PAPER_FRAME做坐标调试 |
| 2 | 已START，正在采集 | 等待结果，不沿用旧方案 |
| 3 | 正在求解 | 等待结果，不沿用旧方案 |
| 4 | 结果就绪 | 允许接收并校验本会话PUZZLE_RESULT |

视觉应用每次启动生成一个随机非零`session_id`，本次运行期间保持不变；`uptime_ms`回绕不会改变会话。F4必须先收到并接受一个合法HEARTBEAT，才允许提交本会话的PAPER_FRAME或PUZZLE_RESULT。首次心跳或`session_id`变化时，F4必须清空旧的`(TYPE, SEQ)`去重记录并作废尚未执行的旧待执行方案，然后保存新会话ID；否则Maix重启后从小序号重新发送时会被误判为旧帧。

Maix每500ms产生一个新心跳序号。同一轮发送中HEARTBEAT先于已经排队的PAPER_FRAME或PUZZLE_RESULT写入UART。F4只要心跳版本、10字节长度、字段和CRC正确，就应更新会话并立即ACK；Maix在1500ms内收到匹配ACK时显示`UART:OK`，否则显示`UART:OFFLINE`。
...
F4必须维护`current_session_id`、`heartbeat_seen`以及当前会话最近已接受的`(TYPE, SEQ)`去重表：

1. 收到合法10字节HEARTBEAT：若尚无会话或`session_id`改变，先清空旧去重表和旧待执行方案，再保存新会话；随后发送`status=0` ACK。
2. 尚未收到合法HEARTBEAT时收到`PAPER_FRAME`或`PUZZLE_RESULT`：不得保存或执行，发送非0 ACK，建议`status=2`表示会话未就绪。
3. 当前会话第一次收到合法`PAPER_FRAME`或`PUZZLE_RESULT`：保存一次，然后发送`status=0` ACK。
4. 当前会话再次收到相同`TYPE+SEQ`：只重发`status=0` ACK，不得再次保存方案或启动电机动作。
5. 收到CRC、VERSION、LENGTH不合法的帧：丢弃且不ACK，因为无法证明引用字段可靠。
6. 收到CRC正确但载荷字段非法的帧：不执行，发送非0 ACK；Maix会保留同一逻辑消息继续重发，便于调试发现错误。
7. 新的PUZZLE_RESULT可替换旧待执行方案；F4执行前仍需自行检查电机软限位、急停和夹具状态。
...
while 环形缓冲有数据:
    ...
    4. VERSION != 2 或 LENGTH > 64：只丢弃当前AA，重新搜索
    5. 少于11 + LENGTH字节时等待下一批数据
    6. 计算CRC(偏移2, 长度7 + LENGTH)，与末尾小端CRC比较
    7. CRC错误：只丢弃当前AA，重新搜索，不能清空整个环形缓冲
    8. CRC正确：按TYPE和严格载荷长度解析
    9. HEARTBEAT先处理会话切换；业务帧先检查heartbeat_seen，再执行本会话去重
   10. 业务字段全部合法后原子提交数据并发送ACK
   11. 从环形缓冲移除本帧11 + LENGTH字节，继续解析后续粘包
```
...
```c
/* F4启动时current_session_id=0、heartbeat_seen=false，并清空去重表。 */
static uint32_t current_session_id;
static bool heartbeat_seen;

/* 伪代码：frame已经完成帧头、版本、通用长度上限和CRC校验。 */
static void vision_handle_frame(const VisionFrame *frame)
{
    if (frame->type == VISION_TYPE_HEARTBEAT) {
        ...
        uint32_t session_id = vision_read_u32_le(&frame->payload[0]);
        uint8_t app_state = frame->payload[8];
        if ((session_id == 0U) || (app_state > 4U)) {
            vision_send_ack(frame->type, frame->sequence, 1U);
            return;
        }

        if ((!heartbeat_seen) || (session_id != current_session_id)) {
            /* 新视觉进程的SEQ可能从头开始，旧会话去重记录必须失效。 */
            vision_clear_duplicate_cache();
            vision_discard_pending_plan();
            current_session_id = session_id;
        }
        heartbeat_seen = true;
        vision_update_remote_state(
            app_state,
            frame->payload[9],
            vision_read_u32_le(&frame->payload[4]));
        vision_send_ack(frame->type, frame->sequence, 0U);
        return;
    }

    if (!heartbeat_seen) {
        /* 首个合法心跳前禁止让任何业务帧进入机械状态。 */
        vision_send_ack(frame->type, frame->sequence, 2U);
        return;
    }
...
启动会话`session_id=0x12345678`、运行1000ms、正在求解`app_state=3`、SEQ=`0x0001`：

```text
AA 55 02 01 01 01 00 0A 00
78 56 34 12 E8 03 00 00 03 00
64 66
```

其中载荷前4字节是会话ID，后4字节是`uptime_ms=1000`；末尾CRC小端字节`64 66`表示数值`0x6664`。
...
```text
AA 55 02 80 00 44 00 04 00 20 20 00 00 1C DE
```

## 11. 联调步骤

1. 先只连接GND和交叉TX/RX，F4电机输出保持禁用。
2. 运行Maix程序，确认F4每约500ms收到10字节HEARTBEAT；记录非零`session_id`，逻辑分析仪解码必须为115200 8N1。
3. F4处理首个心跳、建立会话并发匹配ACK，Maix屏幕应在1500ms内从`UART:OFFLINE`变为`UART:OK`。
...
9. 保持F4运行并单独重启Maix，确认新`session_id`使F4清空旧去重表；新会话首个合法心跳之前发送的业务测试帧不得进入机械状态。
10. 最后才启用电机，先抬高夹具、低速、单片验证坐标和旋转正负，再运行全部碎片。
