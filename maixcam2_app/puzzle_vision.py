...
def compute_piece_geometry(contour, roi, config):
    ...
    关键参数：contour 使用原图坐标，roi 为原图工作区，config 为完整视觉配置。
...
def detect_pieces(frame_bgr, roi, config=None):
    ...
    返回值：``DetectionResult``，其中轮廓坐标已经转换回原图坐标系。
    """
    merged_config = dict(DEFAULT_CONFIG)
    if config:
        merged_config.update(config)

    mask, threshold = build_foreground_mask(frame_bgr, roi, merged_config)
    ...
    min_area = roi_area * float(merged_config["min_area_ratio"])
    max_area = roi_area * float(merged_config["max_area_ratio"])
    ...
    for contour in valid_contours:
        ...
        pieces.append(compute_piece_geometry(global_contour, roi, merged_config))
