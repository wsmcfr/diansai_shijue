...
def build_button_layout(width, height):
    """根据相机显示图像尺寸创建正常识别界面的四个按钮。

    主要流程：从左排列已知、未知和模板保存按钮，并复用右上角CAL切换按钮。
    关键参数：width、height 为处理后显示图像尺寸。
    返回值：以 ``known``、``unknown``、``save``、``cal`` 为键的按钮字典。
    """
    if width <= 0 or height <= 0:
    ...
    return {"known": known, "unknown": unknown, "save": save, "cal": cal}
...
def build_calibration_layout(width, height):
    """创建调参页面的顶部页签和底部参数控制按钮。

    主要流程：顶部从左排列ROI、MASK、RESULT并复用右上角CAL；底部按权重分配
    ITEM、减、数值、加、STEP和SAVE，保证640×480下按钮不重叠且触摸面积稳定。
    关键参数：width、height 为处理后显示图像尺寸。
    返回值：以动作名称为键的十个 ButtonRect 字典。
    """
    if width <= 0 or height <= 0:
        raise ValueError("图像宽高必须大于零")

    margin = max(6, int(round(width * 0.015625)))
    top_gap = max(6, int(round(width * 0.0125)))
    top_height = max(36, min(52, int(round(height * 0.10))))
    roi_width = max(84, int(round(width * 0.14)))
    mask_width = max(92, int(round(width * 0.15)))
    result_width = max(104, int(round(width * 0.17)))

    roi = ButtonRect("roi", margin, margin, roi_width, top_height)
    mask = ButtonRect(
        "mask",
        roi.x + roi.width + top_gap,
        margin,
        mask_width,
        top_height,
    )
    result = ButtonRect(
        "result",
        mask.x + mask.width + top_gap,
        margin,
        result_width,
        top_height,
    )
    cal = build_cal_toggle_button(width, height)
    if result.x + result.width >= cal.x:
        raise ValueError("图像宽度不足以放置调参顶部按钮")

    control_gap = max(4, int(round(width * 0.008)))
    control_height = max(48, min(64, int(round(height * 0.12))))
    control_y = height - margin - control_height
    available_width = width - 2 * margin - 5 * control_gap
    weights = (1.60, 0.85, 1.45, 0.85, 1.25, 1.55)
    weight_total = sum(weights)
    control_widths = [
        int(available_width * weight / weight_total) for weight in weights
    ]
    # 整数截断产生的剩余像素补到最后按钮，确保右边距与左边距一致。
    control_widths[-1] += available_width - sum(control_widths)
    control_names = ("item", "minus", "value", "plus", "step", "save_settings")

    controls = {}
    control_x = margin
    for name, control_width in zip(control_names, control_widths):
